...
//First thing we need to do to tell foundry that this is a script we need to import some additinal code
import {Script} from "forge-std/Script.sol";
import {SimpleStorage} from "../src/SimpleStorage.sol";

contract DeploySimpleStorage is Script{
    //Inside every deploy or script contract we need the main function and this is going to be run
    function run() external returns(SimpleStorage){
        vm.startBroadcast(); //vm is a special keyword that we can only use in Foundry This says everything after this line should go to the RPC
        SimpleStorage simpleStorage = new SimpleStorage(); //This is how we deploy the simpleStorage contract
        vm.stopBroadcast();
